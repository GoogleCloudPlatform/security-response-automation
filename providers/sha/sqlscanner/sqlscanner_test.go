...
func TestReadFindingUpdatePassword(t *testing.T) {
	const (
		...
		noRootPasswordRemediated = `{
			...
			"finding": {
				...
				"securityMarks": {
					"name": "organizations/1055058813388/sources/1986930501971458034/findings/986d52793c4aefc976dd2f35c14b7726/securityMarks",
					"marks": {
						"sra-remediated-event-time": "2019-10-31T22:20:22.425Z"
					}
				},
				"eventTime": "2019-10-31T22:20:22.425Z",
				"createTime": "2019-10-31T22:52:35.630Z"
		...
		errorMessage = "remediation ignored! Finding already processed and remediated. Security Mark: \"sra-remediated-event-time: 2019-10-31T22:20:22.425Z\""
	)
	extractedValues := &updatepassword.Values{
		ProjectID:    "threat-auto-tests-07102019",
		InstanceName: "test-no-password",
		Host:         "%",
		UserName:     "root",
		Mark:         "2019-10-31T22:20:22.425Z",
		Name:         "organizations/1055058813388/sources/1986930501971458034/findings/986d52793c4aefc976dd2f35c14b7726",
	}
	for _, tt := range []struct {
		...
		{name: "read", ruleName: "sql_no_root_password", values: extractedValues, bytes: []byte(noRootPassword), expectedErrMsg: ""},
		{name: "remediated", ruleName: "", values: nil, bytes: []byte(noRootPasswordRemediated), expectedErrMsg: errorMessage},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			if r != nil {
				values, err := r.UpdatePassword()

				if err == nil {
					tt.values.Password = values.Password
...
func TestReadFindingRequireSSL(t *testing.T) {
	const (
		...
		enforceSSLRemediated = `{
			...
			"finding": {
				...
				"securityMarks": {
				  "name": "organizations/119612413569/sources/7086426792249889955/findings/00079ac439b9c80604b895289fd0686c/securityMarks",
				  "marks": {
					"sra-remediated-event-time": "2019-10-25T23:20:25.280Z"
				  }
				},
				"eventTime": "2019-10-25T23:20:25.280Z",
				"createTime": "2019-10-03T17:20:24.389Z"
		...
		errorMessage = "remediation ignored! Finding already processed and remediated. Security Mark: \"sra-remediated-event-time: 2019-10-25T23:20:25.280Z\""
	)
	extractedValues := &requiressl.Values{
		ProjectID:    "sha-resources-20191002",
		InstanceName: "public-sql-instance",
		Mark:         "2019-10-25T23:20:25.280Z",
		Name:         "organizations/119612413569/sources/7086426792249889955/findings/00079ac439b9c80604b895289fd0686c",
	}
	for _, tt := range []struct {
		name           string
		ruleName       string
		values         *requiressl.Values
		bytes          []byte
		expectedErrMsg string
	}{
		{name: "read", ruleName: "ssl_not_enforced", values: extractedValues, bytes: []byte(enforceSSL), expectedErrMsg: ""},
		{name: "remediated", ruleName: "", values: nil, bytes: []byte(enforceSSLRemediated), expectedErrMsg: errorMessage},
	} {
		t.Run(tt.name, func(t *testing.T) {
			r, err := New(tt.bytes)
			if err != nil && err.Error() != tt.expectedErrMsg {
			...
			if r != nil {
				if values := r.RequireSSL(); *values != *tt.values {
...
func TestReadFindingRemovePublic(t *testing.T) {
	const (
		...
		openCloudSQLRequired = `{
			...
			"finding": {
				...
				"securityMarks": {
				  "name": "organizations/119612413569/sources/7086426792249889955/findings/b7a48a4162ca2fb64627dd0a9a9756e1/securityMarks",
				  "marks": {
					"sra-remediated-event-time": "2019-10-11T23:20:26.221Z"
				  }
				},
				"eventTime": "2019-10-11T23:20:26.221Z",
				"createTime": "2019-10-03T17:20:24.331Z"
		...
		errorMessage = "remediation ignored! Finding already processed and remediated. Security Mark: \"sra-remediated-event-time: 2019-10-11T23:20:26.221Z\""
	)
	extractedValues := &removepublic.Values{
		ProjectID:    "sha-resources-20191002",
		InstanceName: "public-sql-instance",
		Mark:         "2019-10-11T23:20:26.221Z",
		Name:         "organizations/119612413569/sources/7086426792249889955/findings/b7a48a4162ca2fb64627dd0a9a9756e1",
	}
	for _, tt := range []struct {
		name           string
		ruleName       string
		values         *removepublic.Values
		bytes          []byte
		expectedErrMsg string
	}{
		{name: "read", ruleName: "public_sql_instance", values: extractedValues, bytes: []byte(openCloudSQL), expectedErrMsg: ""},
		{name: "remediated", ruleName: "", values: nil, bytes: []byte(openCloudSQLRequired), expectedErrMsg: errorMessage},
	} {
		t.Run(tt.name, func(t *testing.T) {
			r, err := New(tt.bytes)
			if err != nil && err.Error() != tt.expectedErrMsg {
			...
			if r != nil {
				if values := r.RemovePublic(); *values != *tt.values {
					t.Errorf("%s failed: got:%v want:%v", tt.name, values, tt.values)
				}
				if name := r.Name(tt.bytes); name != tt.ruleName {
					t.Errorf("%q got:%q want:%q", tt.name, name, tt.ruleName)
