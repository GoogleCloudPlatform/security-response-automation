...
### Preventing retriggering of automations

When Security Response Automation process a finding, it puts a Security Command Center security mark on the finding to indicate that it has been processed.

This prevents retriggering of the configured automation if same exact finding is received a second time. A new detection of the same issue, for example, generated by a new scan, will trigger the automation again.

You will need to grant the role needed to set security marks on findings to the Security Response Automation service account at the organization level:
