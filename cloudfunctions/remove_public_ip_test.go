...
var (
	publicIpAddressFinding = pubsub.Message{Data: []byte(`{
...
func TestRemovePublicIP(t *testing.T) {
	...
	test := []struct {
	...
	}{
		{
			...
			finding:   publicIpAddressFinding,
		},
	}
	for _, tt := range test {
		t.Run(tt.name, func(t *testing.T) {
			ent, computeStub, crmStub := setup(tt.folderIDs)
			computeStub.StubbedInstance = tt.instance
			crmStub.GetAncestryResponse = tt.ancestry
			if err := RemovePublicIP(ctx, tt.finding, ent); err != nil {
...
func setup(folderIDs []string) (*entities.Entity, *stubs.ComputeStub, *stubs.ResourceManagerStub) {
