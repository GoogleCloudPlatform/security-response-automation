...
// Needed contains the entities needed for this function.
type Needed struct {
	Host   *entities.Host
...
func Execute(ctx context.Context, values *Values, ent *Needed) error {
	...
	disks, err := ent.Host.ListInstanceDisks(ctx, values.ProjectID, values.Zone, values.Instance)
	if err != nil {
		return errors.Wrap(err, "failed to list disks")
	}

	snapshots, err := ent.Host.ListProjectSnapshots(ctx, values.ProjectID)
	if err != nil {
	...
	for _, disk := range disks {
		...
		for k := range removeExisting {
			if err := ent.Host.DeleteDiskSnapshot(ctx, values.ProjectID, k); err != nil {
				return errors.Wrapf(err, "failed deleting snapshot: %q", k)
			}
			ent.Logger.Info("removed existing snapshot %q from disk %q", k, disk.Name)
		}

		if err := ent.Host.CreateDiskSnapshot(ctx, values.ProjectID, values.Zone, disk.Name, snapshotName); err != nil {
			return errors.Wrapf(err, "failed creating snapshot: %q", snapshotName)
		}
		ent.Logger.Info("created snapshot for disk %q", disk.Name)

		if err := ent.Host.SetSnapshotLabels(ctx, values.ProjectID, snapshotName, disk, labels); err != nil {
